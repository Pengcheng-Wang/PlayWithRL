...
-- Date: 1/13/17
-- Time: 2:25 PM
-- This script is modified from rlenv_exp7.lua
-- The main change is that, it uses the ConvLSTM and the conv layer setting the same as in Kaxhin Atari project
--
...
cmd = torch.CmdLine()
cmd:option('-rnn_conf', 'rlrnn_conf_rlenv6_1L', 'The RNN layers configuration.')
...
cmd:option('-learning_rate_decay_after',400000,'in number of epochs, when to start decaying the learning rate')
cmd:option('-learning_rate_decay_freq',40000,'frequency of learning rate decay, in number of epochs')
...
cmd:option('-max_epochs',1000000,'number of full passes through the training data')
...
cmd:option('-target_q',20,'The frequency to update target Q network. Set it to 0 if target Q is not needed.')
...
cmd:option('-greedy_ep_end', 0.15, 'The ending value of epsilon in ep-greedy.')
...
cmd:option('-print_freq',80,'frequency of printing result on screen')
cmd:option('-write_every', 320, 'Frequency of writing models into files.')
cmd:option('-gc_freq',50,'frequency of invoking garbage collection')
cmd:option('-q_update_freq',4,'frequency of invoking garbage collection')
...
cmd:option('-backward_dropout',17.15,'The dropout rate in backward propagation')
cmd:option('-backward_dropout_thres',17,'The backward dropout will not be invoked if backward_dropout >= this value')
cmd:option('-backward_prop_steps',6,'The number of backprop steps to take')
cmd:option('-backward_prop_not_before',12,'The earliest time step form where backprop should reach to')
cmd:option('-backward_prop_trm_freq',0.5,'The probability of rnn backprop starts from the terminal state')

opt = cmd:parse(arg)
torch.manualSeed(opt.seed)

local _, RlenvGame = pcall(require, opt.RL_env)
local LSTM_model = require 'model.ConvLSTM2'
...
if string.len(opt.init_from) > 0 then
    ...
    -- LSTM_model model
    protos = {}
    protos.rnn = LSTM_model.convlstm(num_actions, rnnConf, opt.dropout, convArgs)
end
...
function feval(network_param)
    ...
    -- construct transition indices where error will be propagated through, and dw will be calculated.
    -- This is one way of doing updating, as introduced in DRQN paper, for purpose of avoiding correlation.
    -- The other way is the one we implemented below, commented as backward dropout.
    local backprop_from = -1    -- This type of updates is the same as DRQN's. Backprop error wrt weight only for a certain consecutive steps
    local backprop_to = 1  -- This should be the earlies time step to backprop to. If used, this value should be smaller than backprop_from
    if opt.backward_dropout >= opt.backward_dropout_thres then  -- If our own backward dropout will not be used
    if torch.uniform() < opt.backward_prop_trm_freq then
        backprop_from = rlTrajLength - 1
    else
        backprop_from = torch.random(opt.backward_prop_not_before, rlTrajLength - 1)
    end
    backprop_to = backprop_from - opt.backward_prop_steps
    end
    ...
    for t = rlTrajLength-1, 1, -1 do
        ...
        local dlst
        if opt.backward_dropout < opt.backward_dropout_thres then   -- If backward dropout is used
        if torch.uniform() < math.exp(-1 * opt.backward_dropout * t) then
        ...
        if t <= backprop_from and t > backprop_to then  -- If in the range, do backprop wrt weights
        dlst = clones.rnn[t]:backward( { obs_train[t], unpack(rnn_state[t-1]) }, drnn_state[t] )  -- the 2nd param in module.backward() is dloss/d_output. In rnn, this rnn_output contains real output and hidden state values.
        else    -- otherwise, only calculate derivative wrt inputs, even though it is actually not necessary here
        dlst = clones.rnn[t]:updateGradInput( { obs_train[t], unpack(rnn_state[t-1]) }, drnn_state[t] )  -- only calculate dloss/dinput, do not calculate dloss/dw
        end
        end
