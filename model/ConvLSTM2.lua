...
local ConvLSTM2 = {}
function ConvLSTM2.convlstm(output_size, rnn_size_conf, dropout, convArgs)
    --- Param meaning:
    ...
    for L = 1, rnn_layers do
        ...
        local reshaped = nn.Reshape(4, rnn_size_conf[L])(all_input_sums)  -- Got it. nn.Reshape(4, rnn_size_conf[L]) reorganize all data entities in the 2-d tensor into a 3-d tensor.
        --                                                        -- The 2nd and 3rd dim are of size 4 and rnn_size_conf[L] respectively. The 1st dim's size of the original entity
        --                                                        -- quantity divided by 4*rnn_size_conf[L]. The organizing of data in new 3-d tensor is along the row priority of
        --                                                        -- the previous 2-d tensor. So, all original data were cut into 4*rnn_size_conf[L] blocks, along row direction.
        ...
        -- decode the write inputs, in_transform is the candidate hidden nueron
    ...
    local proj = nn.Linear(rnn_size_conf[#rnn_size_conf], output_size)(top_h):annotate{name='decoder'}
